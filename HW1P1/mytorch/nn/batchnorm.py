...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        """
        self.dLdBb = np.sum(dLdBZ * self.NZ, axis=0, keepdims=True)  # TODO: Sum over the batch dimension.
        self.dLdBW = np.sum(dLdBZ, axis=0, keepdims=True)  # TODO: Scale gradient of loss wrt BatchNorm transformation by normalized input NZ.
        ...
        dNZdM =  - np.power(self.V + self.eps, -0.5) - 0.5 * (self.Z - self.M) * np.power(self.V + self.eps, -1.5) * ( -2 * np.sum(self.Z - self.M, axis = 0) / self.N )  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM, axis = 0)  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ * np.power(self.V + self.eps, -0.5) + 2 * dLdV * (self.Z - self.M) / self.N + dLdM / self.N  # TODO: Compute gradient of loss with respect to the input.
